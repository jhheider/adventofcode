...
pub fn main() {
  ...
  println!("Day 7: Test 1: Least fuel ({}) used to reach postition {}", test1.1, test1.0);

  let part1 = least_fuel(&input, false);
  assert_eq!(part1, (342, 325528));
  println!("Day 7: Part 1: Least fuel ({}) used to reach postition {}", part1.1, part1.0);

  let test2 = least_fuel(&test, true);
  assert_eq!(test2, (5, 168));
  println!("Day 7: Test 2: Least fuel ({}) used to reach postition {}", test2.1, test2.0);

  let part2 = least_fuel(&input, true);
  assert_eq!(part2, (460, 85015836));
  println!("Day 7: Part 2: Least fuel ({}) used to reach postition {}", part2.1, part2.0);
}
...
fn least_fuel(input: &[i32], part2: bool) -> (i32, i32) {
  ...
  for position in *lowest..=*highest {
    ...
    if cost < output.1 { output = (position, cost); }
  }
