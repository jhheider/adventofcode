...
pub fn main() {
    ...
    assert_eq!(test1, 4);
    println!("Day 18: Test 1: input has {} lights after {} iterations", test1, 4);

    let part1 = solve(input.clone(), 100, false);
    assert_eq!(part1, 768);
    println!("Day 18: Part 1: input has {} lights after {} iterations", part1, 100);

    let test2 = solve(test, 5, true);
    assert_eq!(test2, 17);
    println!("Day 18: Test 2: input has {} lights after {} iterations", test2, 5);

    let part2 = solve(input, 100, true);
    assert_eq!(part2, 781);
    println!("Day 18: Part 2: input has {} lights after {} iterations", part2, 100);
...
fn solve(input: String, iterations: u32, part2: bool) -> u32 {
    ...
    for (x, line) in input.lines().enumerate() {
        for (y, cell) in line.chars().enumerate() {
            if part2 &&
                (x == 0 || x == (size as usize) - 1) &&
                (y == 0 || y == (size as usize) - 1) {
